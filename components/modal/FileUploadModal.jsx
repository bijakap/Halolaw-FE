...
const FileUploadModal = ({active, setActive, title, message, confirmText, cancelText, cancelColor, confirmColor}) => {
    ...
    <div className={active ? "hidden" : 'fixed z-10 inset-0 overflow-y-auto bg-black/30'}>
      <div className="flex items-center justify-center h-screen w-screen">
        <div className="rounded-lg bg-white w-[756px] p-10" style={{ boxShadow : "0px 12px 24px rgba(37, 42, 49, 0.24), 0px 8px 60px rgba(37, 42, 49, 0.32)" }}>
          <div>
            <p className="mb-2 font-['Inter'] text-[#4A4A68] text-[16px] leading-5 tracking-[0.15px] font-semibold">{title}</p>
            <p className="font-['Inter'] text-[#667085] text-[12px] leading-[18px] tracking-[0.4px] font-normal">{message}</p>
          </div>
          ...
          <div className="flex justify-between mt-10 gap-8">
            <button className="w-full rounded-lg py-[10px] px-6 text-[#667085]" style={{ backgroundColor : cancelColor }} onClick={() => setActive(!active)}>{cancelText}</button>
            <button className="w-full rounded-lg py-[10px] px-6 text-white" style={{ backgroundColor : confirmColor }} onClick={() => setActive(!active)}>{confirmText}</button>
          </div>
        </div>
      </div>
    </div>
